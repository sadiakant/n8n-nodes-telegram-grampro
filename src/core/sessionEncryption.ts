...
export class SessionEncryption {
	...
	static isEncryptedSession(session: string): boolean {
		...
		if (session.startsWith('1')) {
			try {
				const s = new StringSession(session);
				// If it successfully parses as a StringSession and has an address, it is a plain session
				if (s.serverAddress) {
					return false;
				}
			} catch {
				// Fall back to legacy encrypted check if parsing fails
			}
		}
		...
		try {
			...
			// Legacy encrypted sessions must be at least 58 bytes
			// The % 2 check is removed as it causes false positives with valid plain sessions
