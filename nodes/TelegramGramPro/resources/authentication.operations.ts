...
function getTelegramClassName(value: unknown): string | undefined {
...
async function buildQrBinary(loginUrl: string, fileNamePrefix: string) {
	const buffer = generateQrPngBuffer(loginUrl, {
		margin: 4,
		width: 320,
	});
	const fileName = getQrPngFileName(fileNamePrefix);
	return {
		buffer,
		fileName,
		mimeType: 'image/png',
...
async function requestQrLogin(this: IExecuteFunctions, i: number): Promise<INodeExecutionData[]> {
	...
	try {
		...
		if (!(result instanceof Api.auth.LoginToken)) {
			throw new Error(`Unexpected QR token response: ${getTelegramClassName(result) ?? 'unknown'}`);
		}
