		...
		try {
			...
			if (mappedError.code === 'FLOOD_WAIT') {
				...
				throw createNodeOperationError(
					`${mappedError.userMessage} Max retries (${maxRetries}) exceeded.`,
					{ cause: err },
				);
			}
			...
			if (mappedError.code === 'AUTH_KEY_DUPLICATED') {
				...
				throw createNodeOperationError(mappedError.userMessage, { cause: err });
			}
			...
			if (mappedError.code === 'NETWORK_TIMEOUT') {
				if (retryCount <= maxRetries) {
					...
					throw createNodeOperationError('Network timeout after multiple retries', {
						cause: err,
					});
				}
			}
