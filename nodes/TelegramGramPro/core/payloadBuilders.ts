...
async function toBuffer(value: unknown): Promise<Buffer | null> {
    ...
    if (typeof value === 'object' && value !== null && 'on' in value && typeof (value as { on: unknown }).on === 'function') {
    ...
    if (typeof value === 'object' && value !== null && 'getReader' in value && typeof (value as { getReader: unknown }).getReader === 'function') {
        const reader = (value as { getReader: () => { read: () => Promise<{ done: boolean; value?: Uint8Array }> } }).getReader();
...
function inferFileName(
    ...
    if (messageType === 'video') return `video_${messageId}${getExtensionFromMimeType(mimeType) || '.mp4'}`;
...
function detectChatType(
    ...
    if (chatEntity instanceof Api.Channel || chatEntity instanceof Api.ChannelForbidden) return chatEntity.broadcast ? 'channel' : 'supergroup';
    if (message.peerId instanceof Api.PeerUser) return senderEntity instanceof Api.User && senderEntity.bot ? 'bot' : 'user';
...
function isPrivateChatType(chatType: TelegramTriggerChatType): boolean { return chatType === 'user' || chatType === 'bot'; }
function isGroupChatType(chatType: TelegramTriggerChatType): boolean { return chatType === 'group' || chatType === 'supergroup'; }
function isChannelChatType(chatType: TelegramTriggerChatType): boolean { return chatType === 'channel'; }

function getEntityLabel(entity: TelegramEntity | undefined): string | null {
    if (entity instanceof Api.Channel || entity instanceof Api.ChannelForbidden || entity instanceof Api.Chat || entity instanceof Api.ChatForbidden) return entity.title ?? null;
...
function fallbackSenderName(message: Api.Message, chatEntity: TelegramEntity | undefined): string | null {
    if (message.isChannel && !message.senderId) return getEntityLabel(chatEntity) ?? normalizeString(message.chatId);
...
function getMessageText(message: Api.Message): string { return message.message ?? message.text ?? ''; }
