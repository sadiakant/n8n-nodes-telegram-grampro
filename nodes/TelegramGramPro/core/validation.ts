 ...
 */
export function validateSessionString(session: unknown): ValidationResult {
 ...
 * Validate chat ID
 */
export function validateChatId(chatId: unknown): ValidationResult {
	const result: ValidationResult = {
		isValid: true,
		errors: [],
		warnings: [],
 ...
 * Validate message ID
 */
export function validateMessageId(messageId: unknown): ValidationResult {
	const result: ValidationResult = {
		isValid: true,
		errors: [],
		warnings: [],
	...
	switch (operation) {
		...
		case 'copyMessage':
			...
			if (!params.saveToSavedMessages && !params.targetChatId) {
				result.errors.push(
					`Target Chat ID or Saved Messages toggle is required for ${operation} operation`,
				);
				result.isValid = false;
			}
			break;
		...
		case 'copyRestrictedContent':
			...
			if (!params.saveToSavedMessages && !params.targetChatId) {
				result.errors.push(
					'Target Chat ID is required for copyRestrictedContent operation when not saving to Saved Messages',
				);
				result.isValid = false;
			}
			break;
		...
		case 'getProfilePhoto':
			...
			if (!params.myProfilePhotoOnly && !params.userId) {
				result.errors.push(
					'User ID is required for getProfilePhoto operation when My Profile Photo Only is disabled',
				);
				result.isValid = false;
			}
			break;
